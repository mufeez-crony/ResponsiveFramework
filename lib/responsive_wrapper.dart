...
class ResponsiveWrapperState extends State<ResponsiveWrapper>
  ...
  EdgeInsets getScaledPadding() {
    ...
    scaledLeftPadding = max(
      0.0,
      getScaledViewPadding().left - getScaledViewInsets().left,
    );
    scaledTopPadding = max(
      0.0,
      getScaledViewPadding().top - getScaledViewInsets().top,
    );
    scaledRightPadding = max(
      0.0,
      getScaledViewPadding().right - getScaledViewInsets().right,
    );
    scaledBottomPadding = max(
      0.0,
      getScaledViewPadding().bottom - getScaledViewInsets().bottom,
    );
...
enum ResponsiveBreakpointBehavior { RESIZE, AUTOSCALE, AUTOSCALEDOWN, TAG }
...
class ResponsiveBreakpointSegment {
  ...
  ResponsiveBreakpointSegment merge(
    ...
    if (responsiveBreakpointSegment.isTag && !isTag) {
      return copyWith(
        responsiveBreakpoint: responsiveBreakpoint.merge(
          responsiveBreakpointSegment.responsiveBreakpoint,
        ),
      );
    }
...
List<ResponsiveBreakpointSegment> getBreakpointSegments(
  ...
  List<ResponsiveBreakpoint> breakpointTags = breakpointsHolder
      .where((element) => element.isTag)
      .toList();
  breakpointsHolder = breakpointsHolder
      .where((element) => !element.isTag)
      .toList();
  ...
  ResponsiveBreakpoint? minWidthOverride = breakpointsHolder.firstWhereOrNull(
  ...
  for (int i = 0; i < breakpointsHolder.length; i++) {
    ...
    if (breakpointSegments.last.breakpoint ==
      ...
      breakpointSegments[breakpointSegments.length - 1] = breakpointSegments
          .last
          .merge(breakpointSegmentHolder);
      continue;
    }
  ...
  for (int i = 0; i < breakpointTags.length; i++) {
    ...
    ResponsiveBreakpointSegment breakpointSegmentHolder = breakpointSegments
        .reversed
        .firstWhere(
          (element) => element.breakpoint <= breakpointTag.breakpoint,
        );
    int breakpointHolderIndex = breakpointSegments.indexOf(
      breakpointSegmentHolder,
    );
